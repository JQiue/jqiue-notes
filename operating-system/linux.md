...
+ 定位：系统运行的最底线软件包。
+ 内容：Linux 内核、systemd、pacman、glibc、网络基础工具。
+ 特点：重要性极高，稳定性要求极其严格。如果这里出问题，系统可能无法引导。

2. [extra] - 常用扩展

- 定位：官方维护的主流应用软件。
- 内容：桌面环境 (GNOME/KDE)、办公套件、浏览器、主流编程语言、驱动程序。
- 特点：容量最大，包含了大部分你能在官方搜到的软件。

3. [multilib] - 32位兼容

+ 定位：在 64 位系统上运行 32 位程序所需的库。
+ 用途：主要为了支持 Steam、游戏、Wine 以及部分老旧驱动。
+ 开启：需要在 /etc/pacman.conf 中手动取消注释。

4. [testing] - 测试前沿

+ 定位：软件包进入正式仓库前的“候补区”。
+ 风险：极高，仅供测试 Bug 使用。普通用户除非为了尝鲜或解决紧急 Bug，否则不建议开启。
+ 在 Arch Linux 中，有几个重要的仓库：

5. AUR (Arch User Repository)

+ 本质：用户上传的打包脚本 (PKGBUILD)，而非直接的安装包。
+ 特点：软件量极其恐怖，几乎能找到任何小众软件。
+ 工具：需要使用 yay 或 paru 等助手来简化“下载->编译->安装”的过程。

6. ArchLinuxCN (中文社区仓库)

+ 本质：由国内社区维护的二进制仓库。
+ 优势：包含大量国内软件（如微信、QQ、腾讯会议），且预编译了 yay 等工具，省去了手动编译 AUR 的麻烦。
