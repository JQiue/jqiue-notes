...
ES6 前不支持只读变量的定义，但可以通过`Object.defineProperty()`间接实现定义，后在 ES6 中支持以`const`关键字定义只读变量，它和`let`成为了声明变量的主要方式

```js
const PI = 3.14;
```

ES6 中规定，但凡在块级作用域使用`let/const`，这个区域中的变量就绑定到这个区域，不会受到外界影响
...
+ 只要具有`Symbol.iterator`就是可迭代的，比如数组、字符串、Set/Map、否则就是不可迭代的，要手动实现迭代器方法
