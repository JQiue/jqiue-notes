...
在 HTTP 中不存在连接的概念，只有请求和响应，产生的数据传输必须有一个通道，这就不是 HTTP 做的事情了，因此必须先了解一下 TCP 连接
...
无状态的 HTTP 会减少性能开销，但是又要解决类似的矛盾问题，于是引进了 Cookie 技术，即在请求和响应报文中写入 Cookie 信息来管理状态
