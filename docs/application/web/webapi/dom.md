...
`window.document`对象描述了整个网页，它是页面的入口点，也是 DOM 树最顶层的节点
...
1. 元素节点：标签被称为元素节点，并形成了树结构，`<html>`是根节点，`<head>`和`<body>`都是其子节点
2. 文本节点：标签中的文本形成文本节点，一个文本节点只包含一个字符串，没有子节点，它永远都是树的叶子

注意空格和回换行符都是有效的字符，它们也会形成文本节点成为 DOM 树中的一部分

只有两个地方被排除：

1. `<head>`之前的空格和换行符会被忽略
2. 在`</body>`之后的一些东西，都会移动到`<body>`内的最下方，因为规范中要求所有的内容必须位于`<body>`内

所以如果文档中有空格，它也会成为树中的文本节点，如果删除它们，则不会有任何空格，比如将代码改写成下面的样子：
...
即使不提供`<html>`，浏览器也会创建它，`<body>`也是如此

包括一些没有闭合的标签，浏览器都会帮助修正，让它成为正确的 DOM

## 节点类型

每一个节点都属于某种类型，甚至连 HTML 中的注释都会成为 DOM 中的一部分，当然最开头的`<!DOCTYPE HTML>`也是，DOM 定义了 12 种节点类型，一般用到的只有 4 种
...
`nodeType`会返回该节点类型对应的数值，别忘了有 12 中节点类型：

+ 对于元素节点`elem.nodeType == 1`
+ 对于文本节点`elem.nodeType == 3`
+ 对于文档节点`elem.nodeType == 9`

如果知道一个节点，可以通过`nodeName`或`tagName`属性读取它的标签名，但是`tagName`仅适用于元素节点，在一个文本节点上使用会返回`undefined`。而`nodeName`适用于任何节点类型，比如文本类型或注释，会返回一个对应节点类型的字符串
...
修改`innerHTML`的值相当于完全替换了，它也会解析字符串中的标签生成对应的 DOM，即使这个标签是不合法的，也会自动修正

::: tip
如果插入的是`<script>`它也会成为 DOM 中的一部分，但是不会执行
:::

另外，可以使用类似于这样的语法`elem.innerHTML +=`来追加更多内容，但是它却是一种完全重写的方式，即使看起来像追加内容一样，它会先移除旧的内容，然后写入新旧结合的内容。这会导致一些问题，元素的状态会被重新加载了
...
`innerHTML`和`outerHTML`只对元素节点有效，对于文本节点可以使用`nodeValue`和`data`，它们几乎是相同的
...
还有一个`textContent`属性用来获得元素中的纯文本，它会裁掉其中的所有标签，只留下文本内容
...
相对于`innerHTML`来说，通过`textContent`写入文本要更加安全，因为它只会按照字面意思处理

另外还有一个特殊的`hidden`属性用来隐藏元素，它和 CSS 中的`display: none;`是等效的，为`true`时就隐藏掉该元素，写法上会更加简洁

## 遍历 DOM

对于 DOM 来说，所有的操作都是从`document`开始，通过它可以访问任何节点，在最顶层的节点种可以直接作为`document`的属性来访问：

1. `document.documentElement`代表`<html>`对应的元素节点
2. `document.head`代表`<head>`对应的元素节点
3. `document.body`代笔`<body>`对应的元素节点
...
`<html>`是整个文档的根元素，使用`document.documentElement`获取根元素对象，节点对象的`childNodes`属性包含所有子节点的类数组，应该使用`for...of`遍历它。DOM 集合只是可读的，不能够通过赋值来更改一个子节点，需要使用其他方法。DOM 集合还是实时的，反映了 DOM 的当前状态，对 DOM 进行节点更改会实时的反映到集合中，

`firstNode`和`lastNode`是访问第一个和最后一个子节点的最方便的形式
...
对于绝大数情况下来说，我们不需要文本节点或注释节点，而是希望操作的是形成页面结构的元素节点，上面对节点访问方式都是不干净的，而下面的节点是干净的元素节点访问方式，只是加了一个词`Element`表示它访问的是元素类型的节点

+ `children`：返回只包含元素类型的子节点
...
不仅如此，某些类型的 DOM 元素还有着特殊的属性，比如表格元素，除了只是上面的属性，还支持下面的特有属性：
...
修改文档即修改 DOM 节点，要创建一个 DOM 节点有两种方法
...
虽然创建了一个节点，但它并不在 DOM 上，所以需要使用插入方法来将它插入到文档的某一个地方，比如`append()`
...
这些方法的参数不仅可以是插入任意 DOM 节点，还可以是字符串，它会被自动转为文本节点
...
如果想要移除一个节点，可以使用`node.remove()`

```html
<body>
  <script>
    let div = document.createElement('div');
    document.body.prepend(div); // 添加到文档中
...
`elem.cloneNode()`会用来创建一个和自身相同的元素，包括特性，当设置为`true`时，就会进行深克隆。`false`只是克隆自身并不包括子元素
...
`document.write()`可以为 HTML 文档添加一些内容，接受一个字符串，并能够解析字符串中的标签，但它是一个非常古老的方法，一旦调用就会立即写入页面，它只会在页面加载时工作，所以如果进行延迟调用，它会擦除现有的所有文档内容，由于不涉及 DOM，运行非常快
...
大部分 DOM 属性都是字符串类型，但是也有少部分不是，比如`input.checked`是布尔型。还有一个特别例外的情况，`a.href`在 DOM 中永远都是一个完整的 URL 字符串，即使在特性中只有一个相对路径或`#hash`
...
`elem.style`属性是一个对象，这个对象包含了可以为该元素设置的所有样式属性，它们都是这种`{属性名1: 属性值, 属性名2: 属性值, ...}`键值对形式，根据属性名修改属性值就相当于定义了样式
...
这种方式定义的样式就像在 HTML 特性中的`style`一样所写的内容，`elem.style.color = 'red'`等价于`style="color: red;"`。除此之外，对于多词属性，它在`elem.style`中是一种驼峰式，比如`font-size`等于`elem.style.fontSize`。对于一些浏览器专属前缀也是这种风格，`-moz-border-radius`等于`elem.style.MozBorderRadius`

对于一些需要单位的属性值不能够忘记添加到值上，否则会设置失效
...
还有一个非常重要的问题，`elem.style`只和 HTML 中的`style`特性关联，如果元素的属性并不是在这里设置的，那么`elem.style`是无法获取到样式值得
