...
+ 公因数（公约数）：一个数同时是几个整数的因数，有最大公因数
+ 公倍数：一个数是同时是几个整数的倍数，有最小公倍数
...
+ 加法交换律：改变加数的顺序不会改变它们的和
+ 加法结合律：无论先做哪两个加数相加, 都不会改变它们的和
+ 加法恒等律：0 和任意数的和都是那个数
+ 乘法交换律：两个数相乘,交换因数的位置,它们的积不变
+ 乘法结合律：两无论先做哪两个数相乘, 都不会改变它们的结果
+ 乘法恒等律：1 和任意数的积都是那个数
+ 乘法分配律：两个数相乘，其中一个数可以写成两个数的和，并且结果不变
...
一个数的平方根是一个可以与自己相乘得到原数的数字，平方运算（幂运算）和开平方运算互为逆运算，如果一个数字的平方根是一个整数，那个数字就会被称为**完全平方**
...
”=“一直是个被经常忽略的符号，它能将数学元素和表达式连接起来，才能将复杂的表达式转化为简单的，熟悉的，根据已知条件得到对应的答案。”不等“也是一个非常重要的关系，也是各种数学元素的桥梁

相等是一个非常朴素的概念，如果有两堆东西，它们的数量一样多，就称它们的数量相等
...
平均数与低于平均数的数据的距离之和等于它与高于平均数的数据之和，所以说平均数总是平衡点，比如 {2,3,6,9} 的平均数是 5，那么 2 + 3 = 1 + 4 = 5
