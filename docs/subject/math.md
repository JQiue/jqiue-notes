...
比值与计量单位也紧密相连，比如一天 24 小时。比也与局部和整体关联，比如水果中有五分之二是苹果，可以看作 2:5

单位比率可以用第二个量除以第一个量，常用来解决一些应用问题
...
**近似数**就是与实际非常接近的数，**准确数**就是与实际完全符合的数。在现实生活中很多情况下很难得到准确数，可以使用近似数来表示。近似数与准确数的接近程度，可以用精确度表示，比如 215 和约 200 是精确到百位
...
::: tip
指数为零的乘方都等于 1，0 的任何次幂都是 0，正数的任何次幂都是正数，负数的奇次幂都是负数，偶次幂都是正数
:::

求一个数的平方根的运算，叫做开平方，这个数叫做被开方数。一个数的平方根是一个可以与自己相乘得到原数的数字，平方运算（幂运算）和开平方运算互为逆运算，如果一个数的平方根是一个整数，这个个数被称为**完全平方**

一个正数的平方等于 a，那么这个正数叫做 a 的算术平方根。平方根包括算术平方根，算术平方根是平方根的一种，一个正数有两个平方根，而一个正数的算术平方根只有一种

比如 4^2^ = 16，4 就是 16 的算术平方根，也可以说 16 的算术平方根为 $\sqrt{16}$，读作“根号 16”，16 叫做被开方数。被开方数必须大于等于 0

正数 a 的算术平方根表示$\sqrt{a}$，而正数 a 平方根表示$\pm\sqrt{a}$。比如 $x ^ {2} = 9$，9 的平方根为 3 或 -3，算术平方根为 3。a 是被被开方数，3 是根指数，不能省略

一个数的立方等于 a，这个数就叫做 a 的**立方根**，记作 $\sqrt[3]{a}$

平方根和立方根的性质：

+ 负数没有平方根
+ 正数的平方根是正数，负数的平方根是正数，0 平方根是 0
+ 正数的立方根是正数，负数的立方根是负数，0 立方根是 0
+ 互为相反数的立方根，也互为相反数

用来表示一个非负数的算术平方根的式子，叫做二次根式

乘法法则：$\sqrt{a} \times \sqrt{b} = \sqrt{ab}$，前提是 a 和 b 都大于等于 0
...
正数的正分数指数幂：$a^\frac{m}{n} = \sqrt[n]{a^m}$（a > 0, m, n ∈ N*, 且 n > 1）

正数的负分数指数幂：$a^{-\frac{m}{n}} = \frac{\sqrt[n]{a^m}}{a^m}$（a > 0, m, n ∈ N*, 且 n > 1）
...
数学比语言更简洁，比如表示”比 x 多 3“时，就是 x + 3

用字母表示未知数的数是**代数**，含有未知数的式子叫做**方程**，对于一个方程，为了使等式成立，通常要求出未知数的值，这个值叫做方程的解。对于一个方程通常使用逆运算进行求解，加法和减法互为逆运算，乘法和触发互为逆运算
...
当未知数只有一个，且最高次数时 2 的叫做**一元二次方程**，一般的表现形式为：ax^2^ + bx + c = 0（a≠0）
...
一个方程有两个未知数，但含有未知数的项最高次数为 1 叫做**二元一次方程**，一般的表现形式为：ax + by = c（a≠0，b≠0）。把具有相同未知数的两个二元一次方程合在一起，就组成了一个**二元一次方程组**。即使某一个方程是一元一次，但只要另一个是二元一次方程，也是二元一次方程组。两个方程的公共解，叫做二元一次方程组的解
...
二元一次方程组的解法反映出另一条非常重要的解题思路：联立  

简而言之，一个方程是几元，就看未知数的数量，它们的求解方式都是使用消元法转换到最后的一元求解
...
一元一次不等式组中的解可能是”或“，也可能是”与“。当为”或“时只能满足一个不等式，当为”与“时公共部分可以满足两个不等式
...
斜率是用来测量一条直线倾斜的程度，斜率 = 纵向的变化/横向的变化，只需要两个点就可以求出斜率，水平线的斜率总是 0

对于斜截式方程 y = kx + b，只需要一个确定的点，每个数加上斜率即可知道第二个点并画出直线

截距是直线与纵坐标相交的点，该点与原点的距离就是截距
...
函数是用来描述变化关系的，函数是接受一个输入值，处理后输出一个值，比如 y = x + 1 就是一个函数，y = f(x) = x + 1，其中 x 是函数的自变量，函数的处理过程是 x + 1，y 会随着 x 的变化而变化
...
+ 一次函数（斜截式） ——— $y = kx+b$（k 是常数，斜率，k≠0，b 是截距）
...
+ 列举法：将 x 和 y 的值列举出来，非常具有局限性，不能表达出所有的值
+ 图像法：每一个值代表对应的点，当连接这些点时就可以表达出函数关系
...
在两个非空集合 A 和 B 中，如果存在某种对应关系，使 A 中的每个元素，都能对应到 B 中的每个元素，这种对应关系就叫映射，这种对应关系一般用小写的 f 表示。A 中的元素 a~1~ 在映射关系 f 的作用下所对应 B 中的元素 b~1~，则称 b~1~ 是 a~1~ 的像，a~1~ 是 b~1~ 的原像，记作f(a~1~)=b~1~。在映射 A->B 的关系中，A 中的每个元素都能在 B 中找到它对应的像，并且只有一个。而 B 中不需要每个元素都能在 A 中找到对应的原像，并且可以在 A 中找到不止一个原像

在映射张，有三个特殊的映射：

+ 单射 - 在映射 A -> B 中，B 中的每个元素最多只有一个原像
