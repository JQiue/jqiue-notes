...
若 a^m^ = P，则 log~a~p = m（a > 0 且 a ≠ 1），其中 a 是底数，m 是指数，p 叫做”a 的 m 次幂“，即 p 叫做**真数**。其中 m 在指数运算中叫做指数，而在对数运算中叫做对数，所以 log~a~p 念作”以 a 为底 P 的对数“
...
以底数为自变量的函数叫做幂函数，一次函数和二次函数都是幂函数，基本形式为：f(x) = x^a^

底数不变，以指数为自变量的函数叫做指数函数，基本形式为：f(x) = a^x^

底数不变，以真数为自变量的函数叫做对数函数，基本形式为：f(x) =log~a~x
