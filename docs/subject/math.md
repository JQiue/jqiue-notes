...
分数是把单位“1”分成若干份，分数是由除法产生的

+ 真分数
+ 假分数
+ 带分数

百分数即以 100 为分母的数，小数，百分数，分数之间可以互相转换。百分数常用在打折，销售，税务，增长等场景
...
等式是对于两个式子相等的表述，含有未知数的式子叫做**方程**，对于一个方程，为了使等式成立，通常要求出未知数的值，这个值叫做方程的解
...
对于一个方程通常使用逆运算进行求解，加法和减法互为逆运算，乘法和触发互为逆运算

方程可以对一个问题进行建模

::: tip
中秋节安然买了月饼，她买了 x 个月饼，总共花费 42 元，每块月饼单价是 7 元，符合该问题的方程是：x * 7 = 42
:::
...
一个方程有两个未知数，但含有未知数的项最高次数为 1 叫做**二元一次方程**，一般的表现形式为：ax + by = c（a≠0，b≠0）
