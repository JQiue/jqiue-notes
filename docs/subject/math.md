...
两个比值相等的式子可以构成**比例**，比如 2:3 = 4:6，在比例中，**两外项积等于两内向的积**，这是比例的基本性质。如果已知任何三项，根据这个性质就可以求出未知项
...
数学比语言更简洁，比如表示”比 x 多 3“时，就是 x + 3。对于一个方程，为了使等式成立，通常要求出未知数的值，这个值叫做方程的解。对于一个方程通常使用逆运算进行求解，加法和减法互为逆运算，乘法和除法互为逆运算
...
方程可以对一个问题进行建模，比如：中秋节安然买了月饼，她买了 x 个月饼，总共花费 42 元，每块月饼单价是 7 元，符合该问题的方程是：x * 7 = 42
...
**二元一次方程**两个未知数，但最高次数为 1 的方程，一般形式为：$ax + by = c（a≠0，b≠0）$
