...
::: danger
声明原型的时候参数可以只写数据类型，如果写了标识符，后面
:::

### 函数的静态变量

到目前为止，在函数中使用到的变量一旦程序出了函数体，就会被自动销毁，如果要想保留一个变量的状态就应该使用**静态变量**，它使用`static`关键字声明
...
这发生了什么，因为静态变量的初始化状态只发生一次，就在程序的开始，并且它实际上是一个全局变量，但是只对声明它的函数可见
...
全局变量不是静态变量，不会自动初始化，当然也可以将它定义为全局的静态变量
