...
多页面应用相对来说，是由多个 HTML 文件构成的应用，更新页面时一般是跳转，只有少部分是替换元素。在跳转的时候对用户来说体验是肯定没有单页面应用好的，因为它需要加载另一个页面的资源，肯定没有像“德芙一样丝滑”
...
在 HTML 中，会将文本内容中连续的空格看成一个空格。想要实现多个空格，就必须用到**字符实体**，这有点像传统编程语言中的转义
...
将样式代码写到一个独立的`.css`文件中，在`<head>`中使用`<link>`的`href`属性引入，文档和样式文件相分离，维护性最好，最推荐使用的方式
