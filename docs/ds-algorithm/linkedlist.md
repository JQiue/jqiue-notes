...
链表也是存储有序的元素，但和数组不同的是，元素在内存中地址不一定是连续的，每个元素由一个数据项和指向下一个元素的引用组成，相对于传统的数组容量是无限的，在进行插入和删除元素的时候不需要移动其他元素。但是链表不能像数组那样随机访问，访问某一个元素，必须从表头开始遍历

链表可以使用数组实现，也可以使用其他的方式进行实现

相对于顺序存储的来说，链式存储的存储空间地址是可以连续的，也可以是不连续的，地址并不对应逻辑关系，顺序存储有以下优势：
