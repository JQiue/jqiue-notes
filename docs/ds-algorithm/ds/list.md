...
相对于顺序存储的链式存储来说，存储空间地址是可以连续的，也可以是不连续的，地址并不对应逻辑关系

相对于顺序存储有以下优势：

1. 大小是无限的，不会产生额外的空间浪费
2. 插入和删除元素是高效的，因为它不需要移动额外的元素

劣势：

1. 不允许随机访问，必须从第一个元素按顺序访问元素
2. 存储密度低，需要额外的空间表示存储关系
3. 对缓存不是很友好
