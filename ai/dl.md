...
张量就是 PyTorch 用来存储数据的主要结构，它和 NumPy 里的数组很像。可以把张量想象成一个多维的盒子，里面装着数字。这些数字可以是向量（一维）、矩阵（二维），或者是更高维度的。举个例子，一张彩色图片可以看作是一个三维张量：

+ 第一个维度代表图片的高度
+ 第二个维度代表图片的宽度
+ 第三个维度代表颜色通道（比如红色、绿色、蓝色）

```py
# 创建一个 3 行 3 列的张量，标量为 0
zero_tensor = torch.zeros(3, 3)
# 创建一个 2 行 2 列的张量，标量为 1
one_tensor = torch.ones(2, 2)
# 创建一个 2 行 4 列的随机张量
rand_tensor = torch.rand(2, 4)
# 只有形状相同的张量才可以进行逐元素计算
a_tensor = torch.ones(2, 2)
b_tensor = torch.ones(2, 2)
c_tensor = a + b
```

在 torch 中，通过继承`torch.nn.Module`来定义模型，所有的神经网络模块都应该继承它，一个典型的模型有两个主要的部分：

+ `__init__`方法：定义模型包含所有的层和组件
+ `forward`方法：定义数据流动路径，也就是数据如何通过你定义的层进行计算，从输入走向输出

```py
import torch

# 继承 nn.Module 类来创建自定义模型
class SimpleNet(torch.nn.Module):
    def __init__(self):
        super(SimpleNet, self).__init__()
        # 定义一个全连接层，输入特征是 10 个，输出特征是 1 个
        self.linear = torch.nn.Linear(10, 1)

        # 在 forward 方法中定义数据的前向传播过程，
    def forward(self, x):
        # x 是输入张量
        out = self.linear(x)
        return out
