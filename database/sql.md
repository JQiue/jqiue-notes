...
title: SQL
category: 数据库
tag: ["SQL", "MySQL", "MariaDB", "PostgreSQL", "SQLite"]
article: false
---

SQL 是一种声明式的领域特定语言，专门用于管理数据库。与 C、Java 等命令式语言不同，SQL 侧重于定义所需结果，而非实现步骤。其功能涵盖数据查询、修改、结构定义和访问控制

SQL 是关系型数据库管理系统（RDBMS）共同遵循的行业标准。然而，不同的 RDBMS 会在标准基础上发展出自己的SQL 方言，而非关系型数据库则通常采用其他查询语言
...
`TRUNCATE`删除记录，表、索引、外键、权限全部保留，并且重置 `AUTO_INCREMENT`，速度比`DELETE`快
:::
...
`sqlite_schema` 是一个特殊表, 存储有数据库的元信息, 如表(table), 索引(index), 视图(view), 触发器(trigger), 可通过 select 查询相关信息。在 SQLite 3.33.0 版本之前，这个表的名字是 `sqlite_master`，现在 `sqlite_master` 仍然作为 `sqlite_schema` 的别名以保证向后兼容。
...
2. `sudo -u postgres initdb --locale=C.UTF-8 --encoding=UTF8 -D '/var/lib/postgres/data'`
