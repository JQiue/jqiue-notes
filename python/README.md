...
默认情况下，通过 pip 安装的包会被安装到系统全局环境中，这使得所有的项目都共享同一套包和版本。这在小型项目中可能没问题，但在大型项目或多个项目并存的情况下，容易导致依赖冲突和版本不兼容的问题。为了解决这个问题，Python 提供了虚拟环境的功能。虚拟环境是 Python 解释器的一个独立副本，每个虚拟环境都有自己的 Python 二进制文件、库和脚本目录，与系统全局环境和其他虚拟环境隔离

Python 3.3 及以上通过 venv 模块原生支持虚拟环境，无需额外安装。创建虚拟环境的命令:
...
uv 是用 Rust 编写的非常快的 Python 版本管理器和项目管理器

```sh
uv python install           # 安装最新版 Python
uv python install 3.11 3.12 # 安装特定版本以及多个版本
uv python install pypy@3.12 # 安装其他 Python 解释器实现
...
uv 会自动发现系统已经安装的 Python 版本到可用列表中
