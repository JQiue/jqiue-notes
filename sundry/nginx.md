...
logrotate 是 Linux 系统的一个日志管理工具，用于自动轮换（rotate）、压缩和删除日志文件，以防止日志文件过大占用磁盘空间。它基于 cron 定时任务运行，通常通过 /etc/logrotate.conf 主配置文件和 /etc/logrotate.d/ 目录下的子配置文件来管理特定服务的日志。对于 Nginx 日志切割，logrotate 可以按天、周或日志大小进行分割，并在切割后通知 Nginx 重新打开新日志文件，而无需重启服务

大多数 Linux 发行版（如 CentOS、Ubuntu）默认已安装 logrotate。如果你不确定，可以运行 `logrotate --version` 检查。如果未安装，使用包管理器安装

在`/etc/logrotate.d/`目录下创建一个名为`nginx`的文件，写入以下内容：

```plain
/etc/nginx/log/*.log {
    daily                   # 每天切割一次
    missingok               # 如果日志文件缺失，不报错继续
    rotate 10               # 保留 10 个旧日志文件，超出自动删除
    dateext                 # 旧日志文件名附加日期（如 access.log-20250903）
    compress                # 压缩旧日志（gzip）
    delaycompress           # 总是与 compress 选项一起用，delaycompress 选项指示 logrotate 不要将最近的归档压缩，压缩将在下一次轮循周期进行
    notifempty              # 如果日志为空，不切割
    create 0644 nginx nginx # 创建新日志文件，权限 0644，所有者和组为 nginx
    sharedscripts           # 多个日志文件共享 postrotate 脚本，只执行一次
    postrotate
...
`logrotate -d /etc/logrotate.d/nginx`打印执行流程，不会实际切割，`logrotate -f /etc/logrotate.d/nginx`强制切割验证效果
