...
| 标准 | 发布年份 | 说明                                                             |
| ---- | -------- | ---------------------------------------------------------------- |
| C89  | 1989     | 第一个官方 C 语言标准                                            |
| C90  | 1990     | C89 被 ISO 采纳后的国际版，内容基本一致                          |
| C99  | 1999     | 新增 `//` 注释、`long long`、变长数组（VLA）、`<stdbool.h>` 等   |
| C11  | 2011     | 新增 `_Generic`、`_Static_assert`、多线程支持（`<threads.h>`）等 |
| C17  | 2018     | 又称 C18，主要是对 C11 的缺陷修复和技术勘误，未引入新特性        |
| C23  | 2023     | 引入 nullptr、改进 UTF-8 支持、增强预处理器等                    |

C 在发展初期并没有形成通用的标准，随着时间的发展，在 1989 年发布了第一个标准 C89。为了提升 C 的适应性，在 1999 年发布了 C99 标准，一直持续到了 2011 年 C11 标准发布为止
