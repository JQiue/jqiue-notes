...
遍历一棵树是指访问树的每个节点并对它们进行某种操作的过程，访问树的所有节点方式有：中序、前序、后序、层次：

- 前序遍历 - “前”指的是根节点在左子树和右子树之前被访问：根 -> 左子树 -> 右子树
- 中序遍历 - “中”指的是根节点在左子树和右子树中间被访问：左子树 -> 根 -> 右子树
- 后序遍历 - “后”指的是根节点在左子树和右子树之后被访问：左子树 -> 右子树 -> 根
- 层次遍历 - 从上到下，从左到右，一层一层地访问

如果树有更多的层级，遍历的逻辑依然保持不变，只是每次访问一个节点时，都把它看作是新的“根节点”，然后递归地重复这个过程
