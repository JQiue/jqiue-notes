...
一个命题是一种要么为真要么为假的有意义的语言结构，对一件事做出判断的命题是**简单命题**，由两个或多个简单命题以一定的形式联结在一起就构成了**复合命题**，有“且”和“或”两种联结词

+ 对于命题“a 且 b”来说，只有当 a 和 b 都为真命题时，“a 且 b”才是真命题
+ 对于命题“a 或 b”来说，当 a 或 b 其中一个为真命题时，“a 或 b”才是真命题

针对某个对象“是否存在”作出判断的命题叫做**存在命题**，这类命题通常由“有”、“一些”、“至少有”等短语，这些词叫做**存在量词**，用符号”∃“表示（exist）。只要存在某个成员符合判断就是真命题
...
| 类型     | 定义                             | 示例                                         |
| -------- | -------------------------------- | -------------------------------------------- |
| 原命题   | 选取某个命题                     | P（例如：若 a，则 b）                        |
| 非命题   | 原命题的否定（对整个命题的否定） | ¬P（例如：“若 a，则 b”不成立，或 a 且 非 b） |
| 逆命题   | 调换条件和结论                   | 若 q，则 p                                   |
...
+ 原命题和其非命题（否定）真假性相反（一个为真，另一个必为假）
+ 原命题和逆否命题真假性一致
+ 原命题和逆命题真假性没有必然联系，原命题和否命题也是

### 推论

如果命题 A 成立能推出命题 B 成立（即“若 A，则 B”为真），则称：
...
如果命题 A，B 既满足 A=>B，也满足 B=>A，则 A 是 B 的**充分必要条件**，B 也是 A 的充分必要条件，记作“A<=>B”，互为充分必要条件的命题叫做**等价命题**

如果命题 A，B，C 满足 A=>B 且 B=>C，则满足关系 a=>c，这个从已知命题逐步推出新命题的过程叫做**推演**。这种逻辑推演过程是解决考试题目和实际技术问题最主要的方法
...
**比**表示两个或多个数量之间的关系，通常写作 A:B。其比值是前项除以后项所得的结果（例如 180:60 的比值是 180÷60=3）。比值也可以体现部分与整体的关系（如五分之二是苹果，表示苹果与水果总量的比是 2:5）
...
1. 等式两边同类项的系数对应相等
2. 若将恒等式移项至一边并合并同类项，则所有项的系数均为 0
:::
...
因式分解法并非对所有一元二次方程都适用。配方法适用于所有一元二次方程，并能够由此推导出求根公式。通过求根公式，我们可以得到根的判别式来判断根的性质和数量，以及韦达定理来描述根与系数之间的关系
