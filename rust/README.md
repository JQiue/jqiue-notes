...
除此之外，还有不包含的`[package]`的工作目录，被称为**虚拟清单**（virtual manifest）。对于没有主`package`的场景且希望将所有的`package`组织在一起时非常适合，虚拟清单是不能够添加`dependencies`的，因为本身并不是一个包
...
可以为虚拟清单添加一些元数据，比如版本号、作者等，然后在成员包的`Cargo.toml`中引用这些元数据：
