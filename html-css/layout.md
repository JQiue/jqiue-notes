...
`flex-basis`定义了在分配多余空间之前，项目占据的主轴空间（main size）。浏览器根据这个属性，计算主轴是否有多余空间。它的默认值为 auto，即项目的本来大小，可以设为跟`width`或`height`属性一样的值（比如 350px），则项目将占据固定空间
...
CSS Grid 布局是现代 Web 开发中一个强大且灵活的布局系统，它彻底改变了构建复杂网页界面的方式。告别浮动、定位的烦恼，Grid 能够以二维（行和列）的方式直观地定义页面结构

Grid 布局的核心是网格容器（Grid Container）和网格项（Grid Items）：

+ 网格容器：这是应用 Grid 布局的父元素。通过将 display 属性设置为 grid 或 inline-grid，就将一个 HTML 元素转换成了网格容器
+ 网格项：网格容器的直接子元素会自动成为网格项。这些网格项将根据定义的网格结构进行排列
...
<style>
  .container {
    display: grid;
    grid-template:
      "header header header" 50px /* 定义第一行区域及高度 */
      "nav    main   aside" 1fr /* 定义第二行区域及高度 */
      "footer footer footer" 50px /* 定义第三行区域及高度 */
      / 100px 1fr 150px; /* 定义列宽 */
  }
</style>
```

创建网格的第一步是定义它的行和列，`grid-template` 可以方便的定义网格的行和列以及它们的大小，然后让网格项去占据这些区域

```css
.header {
  grid-area: header; /* 占据名为 'header' 的区域 */
  background-color: pink;
}
.nav {
  grid-area: nav; /* 占据名为 'nav' 的区域 */
  background-color: lightblue;
}
.main {
  grid-area: main; /* 占据名为 'main' 的区域 */
  background-color: lightgreen;
}
.aside {
  grid-area: aside; /* 占据名为 'aside' 的区域 */
  background-color: lightcoral;
}
.footer {
  grid-area: footer; /* 占据名为 'footer' 的区域 */
  background-color: #eee;
}
```

::: tip fr 和 auto
fr 单位（Fraction Unit）是一个非常强大的单位，表示网格容器中可用空间的一个分数。1fr 表示占据一份可用空间，2fr 表示占据两份

auto 让网格项自动占据其内容所需的空间，或者在 fr 单位分配后占据剩余空间
:::

gap 属性用于设置网格项之间的间距，可以设置行间距和列间距
